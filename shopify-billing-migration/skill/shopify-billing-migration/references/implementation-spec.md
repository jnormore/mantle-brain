...
> model**, in the correct **order of operations**. This is the reference bundled
> with the `shopify-billing-migration` skill; the procedure for using it is in
> the skill's `SKILL.md`.
>
> **Out of scope:** *flex billing* (the `$0`-recurring + usage-record technique
> for re-approval-free in-place tier changes). That is a separate system —
> mantle-brain ships it as the `flex-billing` topic. Where the two interact,
> this spec points at it but does not duplicate it.
>
